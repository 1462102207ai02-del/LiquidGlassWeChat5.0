...
static void MMUpdateFloatingBar(UIViewController *vc) {
    ...
    for (NSInteger i = 0; i < 4; i++) {
        ...
        UIImage *baseImage = (i == selectedIndex ? (item.selectedImage ?: item.image) : (item.image ?: item.selectedImage));
        ...
        label.font = [UIFont systemFontOfSize:10.5 weight:(i == selectedIndex ? UIFontWeightSemibold : UIFontWeightRegular)];
        label.text = item.title ?: @"";
        label.textColor = (i == selectedIndex) ? selectedColor : normalColor;

        UIImage *finalImage = baseImage;
        if (finalImage.renderingMode != UIImageRenderingModeAlwaysOriginal && [finalImage respondsToSelector:@selector(imageWithTintColor:renderingMode:)]) {
